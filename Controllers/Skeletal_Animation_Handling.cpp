...
namespace Jaguar
{
	...
	void Transform_Child_Skeletal_Animation_Joint_Buffer(glm::mat4* Joint_Buffer, const Collada::Collada_Skeleton* Skeleton, size_t Index, size_t Parent_Index)
	{
		...
		Joint_Buffer[Index] = Joint_Buffer[Parent_Index] * Joint_Buffer[Index];

		for (size_t W = 0; W < Skeleton->Joints[Index].Child_Joints.size(); W++)
			Transform_Child_Skeletal_Animation_Joint_Buffer(Joint_Buffer, Skeleton, Skeleton->Joints[Index].Child_Joints[W], Index);
	...
	void Transform_Skeletal_Animation_Joint_Buffer(glm::mat4* Joint_Buffer, const Collada::Collada_Skeleton* Skeleton)
	{
		...
		{
			Transform_Child_Skeletal_Animation_Joint_Buffer(Joint_Buffer, Skeleton, Skeleton->Joints[0].Child_Joints[W], 0);
		}
	}
}