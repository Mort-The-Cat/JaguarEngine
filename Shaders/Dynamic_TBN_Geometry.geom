...
uniform vec3 Lighting_Node_Positions[3];	// these are the positions we'll use to interpolate on a vertex-basis
...
void Interpolate_Lighting_Nodes(vec3 Sample_Position)
{
	...
	Inverse_Distance = 1.0f / Inverse_Distance;

	float A, B, C;

	A = (Distances[1] + Distances[2]) * Inverse_Distance;
	B = (Distances[0] + Distances[2]) * Inverse_Distance;
	C = (Distances[0] + Distances[1]) * Inverse_Distance;
